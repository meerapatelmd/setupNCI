...
inferred_ncit_version = ncit['rdf:RDF']['owl:Ontology']['owl:versionInfo']
print("  Inferred NCI Version: " + inferred_ncit_version, flush = True)
...
with open("dev/tmp_neo4j/nci_version.txt", "w") as f: 
   f.write(str(ncit_version)+"\n")

if inferred_ncit_version != ncit_version:
  sys.exit("ThesaurusInferred.owl and Thesaurus.owl are not the same versions.")
  ...
  ncit_nodes.to_csv(os.path.join(base_folder,"node.csv"), index = False, header = True)
  ncit_edges.to_csv(os.path.join(base_folder,"edge.csv"), index = False, header = True) 
